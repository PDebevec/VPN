...
namespace PM {
    namespace DI {
        ...
        void displayIPv4HeaderInfo(UINT8* packetBuffer) {
            std::cout << "----------------------------------------" << std::endl;
            version(packetBuffer);
            headerLen(packetBuffer);
            DSCP(packetBuffer);
            totalLen(packetBuffer);
            identification(packetBuffer);
            falgs(packetBuffer);
            fragmentOffset(packetBuffer);
            TTL(packetBuffer);
            protocol(packetBuffer);
            headerChecksum(packetBuffer);
            sourceIPAddress(packetBuffer);
            destinationIPAddress(packetBuffer);
        }
